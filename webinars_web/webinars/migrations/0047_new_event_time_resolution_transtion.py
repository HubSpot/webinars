...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for kls in [orm.Event, orm.StagedWebexEvent, orm.WebexEventSnapshot]:
            for event in kls.objects.all():
                event._time_starts_at = event.time_starts_at
                event._time_ends_at = event.time_starts_at + event.duration*60*10**6
                event._timezone = event.timezone_starts_at
                event.save()
    ...
    def backwards(self, orm):
        for kls in [orm.Event, orm.StagedWebexEvent, orm.WebexEventSnapshot]:
            for event in kls.objects.all():
                event.time_starts_at = event._time_starts_at
                event.duration = (event._time_ends_at - event._time_starts_at + 30*10**6) / 60*10**6
                event.timezone_starts_at = event._timezone
                event.save()
