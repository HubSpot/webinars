...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        if settings.ENV == 'local':
            orm.Hub.objects.create(
                id=1,
                created_at = sanetime(),
                updated_at = sanetime() )
            webex_account_type = orm.AccountType.objects.filter(name='Webex')[0]
            gtw_account_type = orm.AccountType.objects.filter(name='GoToWebinar')[0]
            orm.Account.objects.create(
                account_type = webex_account_type,
                username = 'SECRET',
                password = 'SECRET',
                email = 'SECRET@hubspot.com',
                extra_username = 'SECRET',
                created_at = sanetime(),
                updated_at = sanetime(),
                hub_id = 1 )
            orm.Account.objects.create(
                account_type = gtw_account_type,
                username = 'SECRET',
                password = 'SECRET',
                email = 'SECRET@hubspot.com',
                extra_username = '??',
    ...
    def backwards(self, orm):
        if settings.ENV == 'local':
            orm.Account.objects.filter(hub__id=1).delete()
            orm.Hub.objects.get(1).delete()
