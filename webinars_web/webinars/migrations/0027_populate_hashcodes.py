...
class Migration(DataMigration):
    def _calc_registrant_hashcode(self, email):
        return int(int(hashlib.md5(email).hexdigest(),16)%2**31)

    def _calc_event_hashcode(self,**kwargs):
    ...
    def forwards(self, orm):
        for m in [orm.StagedWebexEvent, orm.WebexEventSnapshot, orm.Event]:
            for event in m.objects.all():
                event.hashcode = self._calc_event_hashcode(**event.__dict__)
                event.save()
        for m in [orm.StagedWebexRegistrant, orm.WebexRegistrantSnapshot, orm.Registrant, orm.StagedHubSpotRegistrant, orm.HubSpotRegistrantSnapshot]:
            for registrant in m.objects.all():
                registrant.hashcode = self._calc_registrant_hashcode(registrant.email)
                registrant.save()
